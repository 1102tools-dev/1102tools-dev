...
| Agent | Work | Status |
|---|---|---|
| **GovCon Growth Agent** | Opportunities, competitors, recompetes, teaming, agencies, markets, and pricing context | Stable |
| **Market Research Agent** | FAR Part 10 research, commerciality, competition, small-business availability, and market conditions | Stable |
| **Pre-Award Agent** | SOW/PWS development, IGCEs, and approved scope-to-pricing workflows | Stable |
| **Other Transaction Agent** | Milestone-based OT project descriptions, cost analysis, revisions, and recosting | Stable |
| **Acquisition Policy Agent** | Codified rules, RFO status, deviations, rulemakings, comments, and policy impacts | Stable |
...
All five 1102tools agents are stable. They are installed from the GitHub-hosted 1102tools marketplace; see the [latest release](https://github.com/1102tools-dev/federal-contracting-agents/releases/latest) for current package and catalog details. Codex Desktop/CLI and Claude Code in Claude Desktop/CLI are the two maintained client families and four tested surfaces. Install or select the intended agent before giving the natural-language request; ambient routing remains host-controlled and best effort. This GitHub marketplace is the maintained distribution and is not an official platform storefront listing.

## Data access readiness

No 1102tools account is required; some federal providers require a free account or API key. Each agent checks credential presence locally before its workflow menu or routed response. The check never displays, transmits, logs, or validates the value.
...
Older agent builds can present a failed keyed operation as a generic server failure. Update all five agents and, for multi-agent Codex installs, use the complete shared `1102tools-host` MCP configuration.
